...
fn team_member_refs_formats_agent_type() {
    ...
    let members = vec![
        TeamMember {
            ...
            model_provider: None,
            model: None,
            initial_task: "typed task".to_string(),
            background: false,
            worktree: false,
        },
        TeamMember {
            ...
            model_provider: None,
            model: None,
            initial_task: "blank task".to_string(),
            background: false,
            worktree: false,
        },
        TeamMember {
            ...
            model_provider: None,
            model: None,
            initial_task: "none task".to_string(),
            background: false,
            worktree: false,
        },
    ];
...
#[test]
fn team_bootstrap_message_mentions_coordination_contract() {
    let team_id = "demo-team";
    let lead_thread_id = ThreadId::new();
    let message = build_spawn_team_bootstrap_message(
        team_id,
        lead_thread_id,
        "planner",
        Some("architect"),
        ...
        "Define rollout plan",
        &[
            spawn_team::SpawnTeamMemberArgs {
                name: "planner".to_string(),
                task: "Define rollout plan".to_string(),
    ...
    assert!(message.contains("You are `planner` in agent team `demo-team`."));
    assert!(message.contains("Lead thread"));
    assert!(message.contains("`team_task_claim_next`"));
...
async fn spawn_agent_rejects_worktree_outside_git_repo() {
    ...
    let Err(err) = MultiAgentHandler.handle(invocation).await else {
        panic!("spawn_agent should reject worktree outside a git repo");
    };
    assert_eq!(
        err,
        FunctionCallError::RespondToModel(
...
#[tokio::test]
async fn spawn_agent_with_worktree_persists_lease_and_cleans_up_after_restart() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        agent_id: String,
    }

    let (_repo_dir, repo_path) = create_test_git_repo().await;
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    turn.cwd = repo_path;
    ...
    let spawn_output = MultiAgentHandler
        ...
        .await
        .expect("spawn_agent should succeed with worktree");
    let ToolOutput::Function {
    ...
    let lease_path = worktree_lease_path(codex_home.as_path(), agent_id);
    let lease_raw = tokio::fs::read_to_string(&lease_path)
        .await
        .expect("worktree lease should be persisted");
    let persisted_lease: PersistedWorktreeLease =
        serde_json::from_str(&lease_raw).expect("lease should be valid json");

    assert_eq!(persisted_lease.agent_id, agent_id.to_string());
    assert_eq!(
        persisted_lease
            .worktree_path
            .starts_with(worktree_root_dir(codex_home.as_path(), lead_thread_id)),
        true
    );
    assert_eq!(
        tokio::fs::metadata(&persisted_lease.worktree_path)
            .await
            .is_ok(),
        true
    );
    ...
    {
        let mut registry = worktree_leases()
            .lock()
    ...
    MultiAgentHandler
        ...
        .await
        .expect("close_agent should clean up persisted worktree lease");
...
#[tokio::test]
async fn spawn_agent_worktree_hooks_override_git_worktree_lifecycle() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        agent_id: String,
    }
    ...
    let hook_worktree = turn.config.codex_home.join("hook-worktree");
    let marker_path = turn.config.codex_home.join("worktree-hooks.log");
    ...
    let snapshot = manager
        ...
        .config_snapshot()
        .await;
    assert_eq!(snapshot.cwd, hook_worktree);
    assert_eq!(tokio::fs::metadata(&snapshot.cwd).await.is_ok(), true);
    ...
    MultiAgentHandler
        ...
        .await
        .expect("close_agent should clean up hook worktree");
...
#[tokio::test]
async fn resume_agent_reinjects_team_bootstrap_context() {
    let (mut session, turn) = make_session_and_context().await;
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    let session = Arc::new(session);
    ...
    let spawn_output = MultiAgentHandler
        .handle(invocation(
            ...
            function_payload(json!({
                "team_id": "resume-team",
                "members": [
                    {"name": "planner", "task": "Plan the migration", "model": "gpt-5"}
                ]
            })),
        ))
        .await
    ...
    let planner_id = agent_id(&spawn_result.members[0].agent_id).expect("planner thread id");
    let bootstrap_needle = format!(
        "You are `planner` in agent team `{}`.",
        spawn_result.team_id
    );

    async fn count_bootstrap_messages(thread: &crate::CodexThread, needle: &str) -> usize {
        let history = thread.codex.session.clone_history().await;
        history
            .raw_items()
            .iter()
    ...
    let thread = manager
        .get_thread(planner_id)
        .await
        .expect("planner thread should exist");
    let snapshot = thread.config_snapshot().await;
    assert_eq!(snapshot.model, "gpt-5");
    assert_eq!(
        count_bootstrap_messages(&thread, &bootstrap_needle).await,
        1
    );
    ...
    assert_eq!(
        manager.agent_control().get_status(planner_id).await,
        AgentStatus::NotFound
    );

    let resume_output = MultiAgentHandler
        .handle(invocation(
            session.clone(),
            turn.clone(),
            "resume_agent",
            function_payload(json!({"id": planner_id.to_string()})),
        ))
        .await
        .expect("resume_agent should succeed");
    let ToolOutput::Function {
        body: FunctionCallOutputBody::Text(resume_content),
        ..
    } = resume_output
    else {
        panic!("expected function output");
    };
    let resume_result: resume_agent::ResumeAgentResult =
    ...
    let thread = manager
        .get_thread(planner_id)
        .await
        .expect("resumed planner thread should exist");
    let snapshot = thread.config_snapshot().await;
    assert_eq!(snapshot.model, "gpt-5");
    timeout(Duration::from_secs(5), async {
        loop {
            if count_bootstrap_messages(&thread, &bootstrap_needle).await >= 2 {
                break;
            }
    ...
    .await
    .expect("resume should inject team bootstrap again");
    ...
    MultiAgentHandler
        .handle(invocation(
            ...
            turn,
            "team_cleanup",
            function_payload(json!({"team_id": "resume-team"})),
        ))
        .await
...
#[tokio::test]
async fn spawn_team_injects_teammate_bootstrap_context() {
    let (mut session, turn) = make_session_and_context().await;
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    let session = Arc::new(session);
    ...
    let task = "Deliver a 3-step plan for the rollout";
    let spawn_output = MultiAgentHandler
        .handle(invocation(
            session.clone(),
            turn.clone(),
    ...
    for _ in 0..50 {
        ...
        for (index, item) in items.iter().enumerate() {
            let text = serde_json::to_string(item).expect("response item should serialize");
            if bootstrap_index.is_none() && text.contains(&bootstrap_needle) {
                bootstrap_index = Some(index);
            }
            if task_index.is_none() && text.contains(task) {
                task_index = Some(index);
            }
            if bootstrap_index.is_some() && task_index.is_some() {
                break;
            }
        }
        if bootstrap_index.is_some() && task_index.is_some() {
    ...
    assert!(
        bootstrap_index.is_some(),
        "bootstrap context should be injected"
    );
    assert!(task_index.is_some(), "task should be recorded");
    ...
    MultiAgentHandler
        .handle(invocation(
            ...
            turn,
            "team_cleanup",
            function_payload(json!({"team_id": "bootstrap-team"})),
        ))
        .await
        .expect("team_cleanup should succeed");
}
...
async fn team_cleanup_fails_when_teammate_is_active() {
    ...
    assert_eq!(
        message,
        format!(
            "team `{}` is `active` and does not allow team_cleanup",
            spawn_result.team_id
        )
    );
...
async fn spawn_team_wait_team_and_close_team_flow() {
    ...
    assert_eq!(persisted_config.state, PersistedTeamState::Active);
    ...
    assert_eq!(
        ...
        FunctionCallError::RespondToModel(format!(
            "team `{}` is `cleanup_pending` and does not allow wait_team",
...
#[tokio::test]
async fn spawn_team_rejects_worktree_outside_git_repo() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    let non_repo_dir = tempfile::tempdir().expect("temp dir");
    ...
    let Err(err) = MultiAgentHandler.handle(spawn_invocation).await else {
        panic!("spawn_team should reject worktree outside a git repo");
    };
    assert_eq!(
        err,
        FunctionCallError::RespondToModel(
...
#[tokio::test]
async fn team_tools_recover_from_persisted_config_and_worktree_lease_after_restart() {
    let (_repo_dir, repo_path) = create_test_git_repo().await;
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    ...
    let spawn_output = MultiAgentHandler
        .handle(invocation(
            ...
            function_payload(json!({
                ...
                "members": [
                    {"name": "worker", "task": "work the task", "worktree": true}
                ]
            })),
        ))
        .await
    ...
    let spawn_result: SpawnTeamResult =
        serde_json::from_str(&spawn_content).expect("spawn_team result should be json");
    let member_id = agent_id(&spawn_result.members.first().expect("team member").agent_id)
        .expect("member thread id");
    let lease_path = worktree_lease_path(codex_home.as_path(), member_id);
    let lease_raw = tokio::fs::read_to_string(&lease_path)
        .await
        .expect("lease should be persisted");
    let persisted_lease: PersistedWorktreeLease =
        serde_json::from_str(&lease_raw).expect("lease should be valid json");

    remove_team_record(lead_thread_id, &spawn_result.team_id)
        .expect("remove in-memory team record");
    assert_eq!(
        find_team_for_member(codex_home.as_path(), member_id)
            .await
            .expect("find team for member"),
        Some(spawn_result.team_id.clone())
    );

    remove_team_record(lead_thread_id, &spawn_result.team_id)
        .expect("remove rebuilt team record before team_message");
    let message_output = MultiAgentHandler
        ...
        .await
        .expect("team_message should rebuild the team registry");
    let ToolOutput::Function {
    ...
    assert_eq!(message_result.member_name, "worker".to_string());
    assert_eq!(message_result.agent_id, member_id.to_string());

    remove_team_record(lead_thread_id, &spawn_result.team_id)
        .expect("remove rebuilt team record before team_broadcast");
    let broadcast_output = MultiAgentHandler
        ...
        .await
        .expect("team_broadcast should rebuild the team registry");
    let ToolOutput::Function {
    ...
    assert_eq!(broadcast_result.sent.len(), 1);
    assert_eq!(broadcast_result.failed.is_empty(), true);
    assert_eq!(broadcast_result.sent[0].member_name, "worker".to_string());
    assert_eq!(broadcast_result.sent[0].agent_id, member_id.to_string());

    manager
        .agent_control()
        .shutdown_agent(member_id)
        .await
        .expect("shutdown team member");

    remove_team_record(lead_thread_id, &spawn_result.team_id)
        .expect("remove rebuilt team record before wait_team");
    let wait_output = MultiAgentHandler
        .handle(invocation(
            session.clone(),
            turn.clone(),
        ...
        .await
        .expect("wait_team should rebuild the team registry");
    let ToolOutput::Function {
        body: FunctionCallOutputBody::Text(wait_content),
        ..
    } = wait_output
    ...
    assert_eq!(
        crate::agent::status::is_final(&wait_result.member_statuses[0].state),
        true
    );

    remove_team_record(lead_thread_id, &spawn_result.team_id)
        .expect("remove rebuilt team record before close_team");
    {
        let mut registry = worktree_leases()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let removed = registry.remove(&member_id);
        assert_eq!(removed.is_some(), true);
    }
    ...
    let close_output = MultiAgentHandler
        ...
        .await
        .expect("close_team should use persisted config and worktree lease");
    let ToolOutput::Function {
        body: FunctionCallOutputBody::Text(close_content),
        ..
    } = close_output
    ...
    assert_eq!(close_result.closed[0].name, "worker".to_string());
    assert_eq!(close_result.closed[0].ok, true);
    ...
    MultiAgentHandler
        ...
        .await
        .expect("team_cleanup should remove persisted team state");
    assert_eq!(
        tokio::fs::metadata(team_dir(codex_home.as_path(), &spawn_result.team_id))
            .await
            .is_err(),
        true
    );
...
async fn wait_team_returns_hook_block_for_teammate_idle() {
    ...
    let spawn_result: SpawnTeamResult =
        serde_json::from_str(&spawn_content).expect("spawn_team result should be json");
    let member_id = agent_id(&spawn_result.members[0].agent_id).expect("valid agent id");
    manager
        .agent_control()
        .shutdown_agent(member_id)
        .await
        .expect("shutdown teammate");
    ...
    let mut wait_end_failure_reason = None;
    let _ = timeout(Duration::from_millis(250), async {
        loop {
            ...
            match event.msg {
                ...
                {
                    saw_wait_end = true;
                    wait_end_failure_reason = ev.failure_reason;
                    break;
                }
                _ => {}
            }
    ...
    assert_eq!(saw_wait_end, true);
    assert_eq!(
        wait_end_failure_reason,
        Some("teammate_idle hook 'idle-guard' blocked: blocked teammate".to_string())
    );
}

#[tokio::test]
async fn close_team_partial_close_rejects_incomplete_tasks() {
    let (mut session, turn) = make_session_and_context().await;
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    let session = Arc::new(session);
    ...
    let Err(err) = MultiAgentHandler
        ...
        panic!("partial close should reject incomplete tasks");
    };
    let FunctionCallError::RespondToModel(message) = err else {
        panic!("expected respond-to-model error");
    };
    assert!(message.starts_with("close_team cannot remove members with incomplete tasks:"));
    ...
    MultiAgentHandler
        ...
        .await
        .expect("full close should still succeed");
    MultiAgentHandler
        .handle(invocation(
            session,
            turn,
...
async fn close_team_partial_close_updates_persisted_team_config() {
    ...
    let claim_result: TeamTaskClaimNextResult =
        serde_json::from_str(&claim_content).expect("team_task_claim_next result should be json");
    let planner_task_id = claim_result
        .task
        .expect("planner should have a task")
        .task_id;
    MultiAgentHandler
        .handle(invocation(
            session.clone(),
...
#[tokio::test]
async fn team_task_complete_returns_hook_block() {
    let (mut session, turn) = make_session_and_context().await;
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    let block_script = r#"import sys; sys.stderr.write("blocked completion"); sys.exit(2)"#;
    session.services.hooks = Hooks::new(HooksConfig {
        command_hooks: CommandHooksConfig {
            task_completed: vec![CommandHookConfig {
                name: Some("complete-guard".to_string()),
                command: vec![
                    "python3".to_string(),
                    "-c".to_string(),
                    block_script.to_string(),
    ...
    let Err(err) = MultiAgentHandler
        ...
        panic!("team_task_complete should be blocked by hook");
    };
    assert_eq!(
        err,
        FunctionCallError::RespondToModel(
            "task_completed hook 'complete-guard' blocked: blocked completion".to_string()
        )
    );
    ...
    let list_output = MultiAgentHandler
        ...
        .await
        .expect("team_task_list should still succeed");
    let ToolOutput::Function {
        body: FunctionCallOutputBody::Text(list_content),
        ..
    } = list_output
    ...
    let list_result: TeamTaskListResult =
        serde_json::from_str(&list_content).expect("team_task_list should return json");
    assert_eq!(list_result.tasks.len(), 1);
    assert_eq!(list_result.tasks[0].state, PersistedTaskState::Pending);
}
...
#[tokio::test]
async fn team_message_allows_member_to_message_another_teammate() {
    let (mut session, turn) = make_session_and_context().await;
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    let lead_thread_id = session.conversation_id;
    ...
    let spawn_result: SpawnTeamResult =
        serde_json::from_str(&spawn_content).expect("spawn_team result should be json");
    let planner_id = agent_id(&spawn_result.members[0].agent_id).expect("planner thread id");
    let worker_id = agent_id(&spawn_result.members[1].agent_id).expect("worker thread id");

    manager
        .agent_control()
        .shutdown_agent(worker_id)
        .await
        .expect("shutdown worker should succeed");

    let mut session = unwrap_arc(session_arc, "only one session ref");
    session.conversation_id = planner_id;
    ...
    let message_output = MultiAgentHandler
        .handle(invocation(
            ...
            function_payload(json!({
                "team_id": spawn_result.team_id,
                "member_name": "worker",
                "message": "pair on this"
            })),
        ))
        .await
        .expect("team_message should succeed for teammate sender");
    let ToolOutput::Function {
        body: FunctionCallOutputBody::Text(message_content),
        ..
    } = message_output
    ...
    let inbox_raw = tokio::fs::read_to_string(&inbox_file)
        ...
        .expect("inbox file should exist");
    let entry: inbox::TeamInboxEntry =
        serde_json::from_str(inbox_raw.lines().next().expect("inbox entry"))
            .expect("inbox line should be json");
    assert_eq!(entry.from_thread_id, planner_id.to_string());
    assert_eq!(entry.from_name, Some("planner".to_string()));
    assert_eq!(entry.to_thread_id, worker_id.to_string());
...
async fn team_ask_lead_persists_and_lead_can_pop_and_ack() {
    ...
    if ask_result.delivered {
        assert_eq!(pop_result.messages.is_empty(), true);
        assert_eq!(pop_result.ack_token.is_empty(), true);
    } else {
