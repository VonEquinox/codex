...
pub async fn handle(
    ...
    for result in &mut closed {
        let member_id = super::agent_id(&result.agent_id)?;
        if let Err(err) = cleanup_agent_worktree(session.as_ref(), turn.as_ref(), member_id).await {
            result.ok = false;
            result.error = Some(err.clone());
            cleanup_errors.push(format!("{} ({}): {err}", result.name, result.agent_id));
        }
    }
    if !cleanup_errors.is_empty() {
        return Err(FunctionCallError::RespondToModel(format!(
            "team_cleanup failed to remove teammate worktrees: {}",
            cleanup_errors.join(", ")
        )));
    }
