...
const WORKTREE_LEASES_DIR: &str = "leases";
...
fn worktree_leases_dir(codex_home: &Path) -> PathBuf {
    codex_home.join(WORKTREE_ROOT_DIR).join(WORKTREE_LEASES_DIR)
}
...
fn team_task_path(codex_home: &Path, team_id: &str, task_id: &str) -> std::path::PathBuf {
...
fn persisted_worktree_lease(agent_id: ThreadId, lease: &WorktreeLease) -> PersistedWorktreeLease {
    PersistedWorktreeLease {
        agent_id: agent_id.to_string(),
        repo_root: lease.repo_root.clone(),
        worktree_path: lease.worktree_path.clone(),
        created_via_hook: lease.created_via_hook,
    }
}

fn team_record_from_persisted_config(
    team_id: &str,
    config: PersistedTeamConfig,
) -> Result<(ThreadId, TeamRecord), FunctionCallError> {
    ...
    let members = config
        ...
        .map_err(|err| {
            FunctionCallError::RespondToModel(format!(
                "failed to rebuild team `{team_id}` from persisted config: {err}"
            ))
        })?;
    Ok((
        lead_thread_id,
...
async fn persist_worktree_lease(
    codex_home: &Path,
    agent_id: ThreadId,
    lease: &WorktreeLease,
) -> Result<(), FunctionCallError> {
    let lease_path = worktree_lease_path(codex_home, agent_id);
    write_json_atomic(&lease_path, &persisted_worktree_lease(agent_id, lease))
        .await
        .map_err(|err| {
            FunctionCallError::RespondToModel(format!(
                "failed to write worktree lease for agent `{agent_id}`: {err}"
            ))
        })
}

async fn read_persisted_worktree_lease(
    codex_home: &Path,
    agent_id: ThreadId,
) -> Result<Option<WorktreeLease>, String> {
    let lease_path = worktree_lease_path(codex_home, agent_id);
    let raw = match tokio::fs::read_to_string(&lease_path).await {
        ...
        Err(err) => {
            return Err(format!(
                "failed to read worktree lease for agent `{agent_id}`: {err}"
            ));
        }
    };
    let persisted: PersistedWorktreeLease = serde_json::from_str(&raw)
        .map_err(|err| format!("failed to parse worktree lease for agent `{agent_id}`: {err}"))?;
    Ok(Some(WorktreeLease {
        repo_root: persisted.repo_root,
        worktree_path: persisted.worktree_path,
        created_via_hook: persisted.created_via_hook,
    }))
}

async fn remove_persisted_worktree_lease(
    codex_home: &Path,
    agent_id: ThreadId,
) -> Result<(), String> {
    let lease_path = worktree_lease_path(codex_home, agent_id);
    match tokio::fs::remove_file(&lease_path).await {
        ...
        Err(err) if err.kind() == ErrorKind::NotFound => Ok(()),
        Err(err) => Err(format!(
            "failed to remove worktree lease for agent `{agent_id}`: {err}"
        )),
    }
}
...
async fn get_team_record(
    ...
    registry
        .entry(lead_thread_id)
        .or_default()
        .insert(team_id.to_string(), team_record.clone());
    Ok(team_record)
}

async fn find_team_for_member(
    codex_home: &Path,
...
async fn register_worktree_lease(
    codex_home: &Path,
    agent_id: ThreadId,
    lease: WorktreeLease,
) -> Result<(), FunctionCallError> {
    persist_worktree_lease(codex_home, agent_id, &lease).await?;
...
async fn take_worktree_lease(
    codex_home: &Path,
    agent_id: ThreadId,
) -> Result<Option<WorktreeLease>, String> {
    if let Ok(mut registry) = worktree_leases().lock()
        && let Some(lease) = registry.remove(&agent_id)
    {
        return Ok(Some(lease));
    }
    read_persisted_worktree_lease(codex_home, agent_id).await
}
...
async fn create_agent_worktree(
    ...
    let root = worktree_root_dir(turn.config.codex_home.as_path(), session.conversation_id);
...
async fn cleanup_agent_worktree(
    ...
    let Some(lease) = take_worktree_lease(turn.config.codex_home.as_path(), agent_id).await? else {
        return Ok(());
    };
    match remove_worktree_lease(session, turn, lease.clone()).await {
        Ok(()) => {
            remove_persisted_worktree_lease(turn.config.codex_home.as_path(), agent_id).await?;
            Ok(())
        }
        Err(err) => {
            if let Ok(mut registry) = worktree_leases().lock() {
                registry.insert(agent_id, lease.clone());
            }
            let _ =
                persist_worktree_lease(turn.config.codex_home.as_path(), agent_id, &lease).await;
            Err(err)
        }
    }
}
