...
### TUI hooks manager (`/hooks`)

In the TUI, run `/hooks` to manage hook entries interactively.

- `Global hooks (user)` opens `~/.codex/config.toml`
- `Project hooks (...)` opens the nearest `./.codex/config.toml`
- `Add hook` inserts a single `[[hooks.<event>]]` template
- `Edit raw config.toml` opens the full file for advanced editing

The TUI editor preserves the rest of `config.toml` while mutating `[hooks]`, so unrelated
comments and sections stay intact.
